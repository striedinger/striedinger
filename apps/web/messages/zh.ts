import type { TranslationCatalog } from "@workspace/i18n";

import type { messages as englishMessages } from "./en";

export const messages: TranslationCatalog<typeof englishMessages> = {
  "Hi there!": "你好！",
  "I'm Hugo Striedinger": "我是 Hugo Striedinger",
  "Senior Software Engineer": "高级软件工程师",
  "New York, NY": "纽约州纽约市",
  "Social links": "社交链接",
  "Email Hugo Striedinger": "给 Hugo Striedinger 发邮件",
  "Hugo Striedinger on X": "X 上的 Hugo Striedinger",
  "Hugo Striedinger on Instagram": "Instagram 上的 Hugo Striedinger",
  "Hugo Striedinger on GitHub": "GitHub 上的 Hugo Striedinger",
  "Hugo Striedinger on LinkedIn": "LinkedIn 上的 Hugo Striedinger",
  "Select language": "选择语言",
  Theme: "主题",
  Navigation: "导航",
  "Open navigation menu": "打开导航菜单",
  "Close navigation": "关闭导航",
  "Browser tools": "浏览器工具",
  "Free, privacy-focused browser tools and live utilities.":
    "免费、注重隐私的浏览器工具和实时实用程序。",
  "About Hugo": "关于 Hugo",
  "I build fast, accessible web products and developer tools with a focus on privacy, reliability, and thoughtful user experience.":
    "我构建快速、无障碍的 Web 产品和开发者工具，重视隐私、可靠性与周到的用户体验。",
  "About this tool": "关于此工具",
  "How it works": "工作原理",
  "Privacy and security": "隐私与安全",
  Features: "功能",
  "Live data": "实时数据",
  "Local storage": "本地存储",
  "Open Graph Preview": "Open Graph 预览",
  "IP Address Information": "IP 地址信息",
  "JavaScript Browser Information": "浏览器 JavaScript 信息",
  "JSON Validator and Formatter": "JSON 验证与格式化工具",
  "Image & PDF Optimizer": "图像和 PDF 优化器",
  "Image Optimizer": "图像优化器",
  "PDF Optimizer": "PDF 优化器",
  "Drop - Private file sharing": "Drop - 私密文件共享",
  "Nearby Chat": "附近聊天",
  "Nearby Chat - Private local messaging": "附近聊天 - 私密本地消息",
  "Private Nearby Chat": "私密附近聊天",
  "Private Peer-to-Peer File Sharing": "私密点对点文件共享",
  "Chat privately with nearby devices over a fast, encrypted, serverless peer-to-peer mesh.":
    "通过快速、加密且无需服务器的点对点网状网络与附近设备私密聊天。",
  "Private chat for nearby devices. Messages disappear when you leave.":
    "供附近设备使用的私密聊天。离开后消息即会消失。",
  "Private · Local · Temporary": "私密 · 本地 · 临时",
  "Use a short pairing code to connect nearby browsers. WebRTC carries messages directly between peers.":
    "使用短配对码连接附近的浏览器。WebRTC 在对等设备之间直接传输消息。",
  "Messages are encrypted between connected devices and are not stored after the temporary session ends.":
    "消息在已连接设备之间加密传输，并在临时会话结束后不再存储。",
  "No account is required. Leaving the session clears the temporary conversation from this app.":
    "无需帐户。离开会话后，此应用会清除临时对话。",
  "Share files directly between devices with an encrypted peer-to-peer connection. Nothing is uploaded or stored on this server.":
    "通过加密的点对点连接在设备之间直接共享文件。此服务器不会上传或存储任何内容。",
  "Daily Sudoku": "每日数独",
  "Trains near you": "附近的列车",
  Podcasts: "播客",
  "Stock watchlist": "股票自选列表",
  "Hugo Striedinger - Senior Software Engineer": "Hugo Striedinger - 高级软件工程师",
  "Hugo Striedinger is a Colombian-born senior software engineer based in New York, with experience at SpaceX, Twitter Inc., and X Corp.":
    "Hugo Striedinger 是一位出生于哥伦比亚、现居纽约的高级软件工程师，曾在 SpaceX、Twitter Inc. 和 X Corp. 工作。",
};
